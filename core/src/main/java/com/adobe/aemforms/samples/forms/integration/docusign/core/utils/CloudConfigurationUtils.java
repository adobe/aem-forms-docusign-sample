...
public class CloudConfigurationUtils {
    ...
    /**
     * Read DocuSign cloud configuration resource from /conf
     */
    public static Resource getConfigResource(String cqConfPath, ResourceResolverFactory resourceResolverFactory) {
        Resource configurationResource = null;
        try {
            ResourceResolver resolver = resourceResolverFactory.getServiceResourceResolver(null);
            configurationResource = resolver.getResource(cqConfPath + "/" + Constants.CONFIG_BUCKET + Constants.DOCUSIGN_CONFIGURATION_GROUP);
        } catch (Exception e) {
            logger.error("Exception occurred when reading config ", e);
        }
        return configurationResource != null ?configurationResource.getChild("jcr:content"): null;
    }

    /**
     * Get required tokens by invoking DocuSign Rest API call
     */
