...
public class DocuSignSubmitActionService implements FormSubmitActionService {
    ...
    private HashMap<String, String> newTokens = new HashMap<>();
    ...
    public Map<String, Object> submit(FormSubmitInfo formSubmitInfo) {
        ...
        String formContainerResourcePath = formContainerResource != null ? formContainerResource.getPath() : null;
        ...
        try {
            ...
            logger.info("[AF] [Submit] [DocuSign] recieved new tokens");
        }
        catch (Exception e) {
            logger.error("[AF] [Submit] [DocuSign] [OAuth] Error while fetching Access token for "+ configResource + " Config Resource", e);
        ...
        try (CloseableHttpClient httpclient = httpClientBuilderFactory.newBuilder().build()){
            ...
            if (pdfFile != null) {
                ...
                logger.error("[AF] [Submit] [DocuSign] Document of Record is null for form {}", formContainerResourcePath);
                GuideSubmitUtils.addValidationErrorToResult(result, GuideSubmitErrorCause.FORM_SUBMISSION,
            ...
            request.addHeader("Authorization" , "Bearer "+ newTokens.get(Constants.ACCESS_TOKEN));
    ...
    /*
      Get Access token in configuration
     */
    private void getNewTokens(CloudConfigSlingModel configProperties) throws CryptoException, PersistenceException {
        ...
        newTokens.put(Constants.REFRESH_TOKEN, jsonResponse.get(Constants.REFRESH_TOKEN).getAsString());
        newTokens.put(Constants.ACCESS_TOKEN, jsonResponse.get(Constants.ACCESS_TOKEN).getAsString());
    }
